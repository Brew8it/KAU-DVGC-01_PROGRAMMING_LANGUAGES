...
/*add for 273 and 275 since undef chars wont be recognized*/
...
/*match_char([ ]). match_char([H|T])  :- char_type(H,alpha),match_char(T).*/
...
runparser :- tell('runparser.out'),write('Testing OK programs '),nl, testok,nl,write('Testing a-z programs '),nl, testaz,nl,
          write('Testing fun programs '),nl,testfun,nl,write('Testing sem programs '),nl,testsem,nl,told, 
          write('runparser.out created'), nl, halt.
